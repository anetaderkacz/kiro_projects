...
<div class="box_ogl">
	...
	<div style="position: relative; overflow: hidden;height:200px;margin:5px;" id="naj_new">
		{section name=id loop=$npart_id}
			{if $npr_ii++%5==0}<div style="margin:5px;">{/if}
				<div style="width:175px;float:left;padding:3px;margin-right:5px;border:1px solid #dddddd;" >
					...
					<a href="ogloszenie/{$npart_id[id]}/{$npart_tytuln[id]}" style="font-weight:bold;color:#3184E1;word-wrap: break-word;">{$npart_tytul[id]|substr:0:40}</a>
					</div>
			
					{if $npart_cena[id]<>""} <div style="font-weight:bold;float:left;margin-left:5px;">{$npart_cena[id]} zł</div>{/if}
...
<div> {if $reklama_t<>""}<br>{$reklama_t}{/if}</div>
</div>
