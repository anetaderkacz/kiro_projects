...
$smarty->assign("title",$lang[352].' - '.$ust['nazwa']);

$smarty->display($ust['templates'].'/woj_art.tpl');
...
$Query='SELECT * FROM '.$pre.'woj WHERE w_id="'.db_real_escape_string($_GET['id']).'"'; 
$result = db_query($Query) or die(db_error());
while ($row = db_fetch($result)) 
{
  $cat_nazwa=$row['w_nazwa'];
  $cat_id=$row['w_id'];
}

$zapytanie = "SELECT * FROM ".$pre."artykul WHERE art_akt='1' and art_oplacone='1' AND art_woj='".db_real_escape_string($_GET['id'])."' ".$czy_end_o." ORDER by ".$_SESSION['m_sort_sort']."";
...
$smarty->assign("title",$cat_nazwa.' - '.$lang[353].' - '.$ust['nazwa']);
...
$smarty->display($ust['templates'].'/woj_lista.tpl');
